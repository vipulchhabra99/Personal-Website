...
function _writeRelayGeneratedFile() {
  _writeRelayGeneratedFile = _asyncToGenerator(function*(
    codegenDir,
    _generatedNode,
    formatModule,
    typeText,
    ...
    var printModuleDependency =
      arguments.length > 8 && arguments[8] !== undefined
        ? arguments[8]
        : printRequireModuleDependency;
    var generatedNode = _generatedNode; // Copy to const so Flow can refine.

    var persistQuery = _persistQuery;
    var moduleName =
      (generatedNode.kind === 'Request' ? generatedNode.params.name : generatedNode.name) +
      '.graphql';
    var platformName =
      platform != null && platform.length > 0 ? moduleName + '.' + platform : moduleName;
        ...
        switch (generatedNode.kind) {
          case RelayConcreteNode.REQUEST:
            var _generatedNode$params = generatedNode.params,
              ...
              restParams = (0, _objectWithoutPropertiesLoose2['default'])(_generatedNode$params, [
                'id',
                'text',
              ]);
            !(_text != null)
