...
function visitLinkedField(node, parentType) {
  ...
  if (!(rawType instanceof GraphQLInterfaceType || rawType instanceof GraphQLObjectType)) {
    throw createUserError(
      '@match may only be used on fields whose parent type is an interface ' +
        "or object, field '"
          .concat(node.name, "' has invalid type '")
          .concat(String(parentType), "'"),
      [node.loc],
    );
  }
  ...
  if (jsModuleType == null || !(jsModuleType instanceof GraphQLScalarType)) {
    throw new Error(
      "RelayMatchTransform: Expected schema to define a scalar '".concat(JS_FIELD_TYPE, "' type."),
    );
  }
  ...
  if (
    ...
    throw new Error(
      'RelayMatchTransform: @match used on an incompatible ' +
        "field '".concat(transformedNode.name, "'. @match may only ") +
        "be used with fields that can accept '".concat(SUPPORTED_ARGUMENT_NAME, "' ") +
  ...
  if (!(unionType instanceof GraphQLUnionType)) {
    throw new Error(
      'RelayMatchTransform: You are trying to apply @match ' +
        "directive to a field '".concat(transformedNode.name, "' that has unsupported ") +
        "output type. '".concat(transformedNode.name, "' output type should be union ") +
  ...
  transformedNode.selections.forEach(function(matchSelection) {
    ...
    if (matchSelection.kind !== 'FragmentSpread') {
      throw new Error(
        'RelayMatchTransform: all selections in a @match field should be ' +
          "fragment spreads, got '".concat(matchSelection.kind, "'."),
      );
    }
    ...
    if (!(fragment.type instanceof GraphQLObjectType)) {
      throw new Error(
        'RelayMatchTransform: all fragment spreads in a @match field should ' +
          ...
          "'".concat(fragment.type.name, "' for '...").concat(fragment.name, "' is not supported."),
      );
    }
    ...
    if (seenTypes.has(matchedType)) {
      throw new Error(
        'RelayMatchTransform: Each "match" type has to appear at-most once. ' +
          "Type '".concat(matchedType.name, "' was matched in both ") +
          "'..."
            .concat(matchSelection.name, "' and '...")
            .concat(seenTypes.get(matchedType) || '(unknown)', "'."),
      );
    }
    ...
    if (!belongsToUnion) {
      throw new Error(
        "RelayMatchTransform: Unsupported type '".concat(matchedType.toString(), "' in ") +
          'the list of matches in the @match. Type ' +
          '"'.concat(matchedType.toString(), '" does not belong to the union ') +
          '"'.concat(unionType.toString(), '".'),
      );
    }

    var jsField = matchedType.getFields()[JS_FIELD_NAME];
    var jsFieldArg = jsField
      ? jsField.args.find(function(arg) {
    ...
    if (
      ...
      throw new Error(
        "RelayMatchTransform: expcted type '"
          .concat(matchedType.name, "' to have a '")
          .concat(JS_FIELD_NAME, '(')
          .concat(JS_FIELD_ARG, ': String!): ')
          .concat(JS_FIELD_TYPE, "' field ."),
      );
    }
    ...
    if (moduleDirective == null || matchSelection.directives.length !== 1) {
      throw new Error(
        'RelayMatchTransform: Fragment spreads in a @match field must have a ' +
          ...
          "on fragment spread '...".concat(matchSelection.name, "'"),
      );
    }
      ...
      args: [
        {
          ...
          value: {
            ...
            loc:
              (_ref =
                (_moduleDirective$args = moduleDirective.args[0]) === null ||
                _moduleDirective$args === void 0
                  ? void 0
                  : _moduleDirective$args.loc) !== null && _ref !== void 0
                ? _ref
                : moduleDirective.loc,
            metadata: {},
  ...
  var storageKey =
    ((_transformedNode$alia = transformedNode.alias) !== null && _transformedNode$alia !== void 0
      ? _transformedNode$alia
      : transformedNode.name) + '('.concat(stableArgs.join(','), ')');
  var matchField = {
    kind: 'MatchField',
