...
function compileAll(_ref) {
  ...
  var extendedSchema = ASTConvert.extendASTSchema(
    transformedSchema,
    (0, _toConsumableArray2['default'])(baseDocuments).concat(
      (0, _toConsumableArray2['default'])(documents),
    ),
  ); // Verify using local and global rules, can run global verifications here
  // because all files are processed together

  var validationRules = (0, _toConsumableArray2['default'])(RelayValidator.LOCAL_RULES).concat(
    (0, _toConsumableArray2['default'])(RelayValidator.GLOBAL_RULES),
  );

  if (extraValidationRules) {
    validationRules = (0, _toConsumableArray2['default'])(validationRules).concat(
      (0, _toConsumableArray2['default'])(extraValidationRules.LOCAL_RULES || []),
      (0, _toConsumableArray2['default'])(extraValidationRules.GLOBAL_RULES || []),
    );
  }

  var definitions = ASTConvert.convertASTDocumentsWithBase(
    extendedSchema,
    baseDocuments,
    documents,
    validationRules,
  ...
  var transformedQueryContext = compilerContext.applyTransforms(
    (0, _toConsumableArray2['default'])(compilerTransforms.commonTransforms).concat(
      (0, _toConsumableArray2['default'])(compilerTransforms.queryTransforms),
    ),
    reporter,
  );
...
function writeAll(_ref2) {
  ...
  return Profiler.asyncContext(
    ...
    _asyncToGenerator(function*() {
      ...
      try {
        for (
          var _iterator = (writerConfig.generatedDirectories || [])[Symbol.iterator](), _step;
      ...
      var formatModule = Profiler.instrument(
        writerConfig.formatModule,
        'RelayFileWriter:formatModule',
      );
      var persistQuery = writerConfig.persistQuery
        ? Profiler.instrumentWait(writerConfig.persistQuery, 'RelayFileWriter:persistQuery')
        : null;
      ...
      try {
        ...
        if (sourceControl && !onlyValidate) {
          yield CodegenDirectory.sourceControlAddRemove(
            sourceControl,
