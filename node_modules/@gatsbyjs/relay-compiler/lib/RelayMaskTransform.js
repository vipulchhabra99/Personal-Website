...
function visitFragment(fragment, state) {
  ...
  var joinedArgumentDefinitions = joinFragmentArgumentDefinitions(
    schema,
    fragment,
...
function visitFragmentSpread(fragmentSpread, state) {
  ...
  !!fragment.argumentDefinitions.find(function(argDef) {
    ...
    ? process.env.NODE_ENV !== 'production'
      ...
      : invariant(false)
    : void 0; // Note: defer validating arguments to the containing fragment in order
  ...
  if (errors.length) {
    throw new Error(
      'RelayMaskTransform: Cannot unmask one or more fragments in ' +
        '`'.concat(fragment.name, '`, the following variables are referenced more ') +
...
function joinArgumentDefinition(schema, prevArgDef, nextArgDef) {
  if (
    ...
    prevArgDef.name !== nextArgDef.name || // Only LocalArgumentDefinition defines defaultValue
