...
function print(node) {
  switch (node.kind) {
    case 'Fragment':
      return (
        'fragment '.concat(node.name, ' on ').concat(String(node.type)) +
    ...
    case 'SplitOperation':
      return (
        'SplitOperation '.concat(node.name, ' on ').concat(String(node.type)) +
        printSelections(node, '') +
        '\n'
      );
...
function printSelection(selection, indent) {
  ...
  if (selection.kind === 'LinkedField') {
    ...
    if (selection['if'] !== null) {
      ...
      streamStr += ', if: '.concat(
        (_printValue = printValue(selection['if'])) !== null && _printValue !== void 0
          ? _printValue
          : '',
      );
    }
    ...
    if (selection.initialCount !== null) {
      ...
      streamStr += ', initial_count: '.concat(
        (_printValue2 = printValue(selection.initialCount)) !== null && _printValue2 !== void 0
          ? _printValue2
          : '',
      );
    }
    ...
    if (selection['if'] !== null) {
      ...
      deferStr += ', if: '.concat(
        (_printValue3 = printValue(selection['if'])) !== null && _printValue3 !== void 0
          ? _printValue3
          : '',
      );
    }
    ...
    !false
      ? process.env.NODE_ENV !== 'production'
        ? invariant(false, 'GraphQLIRPrinter: Unknown selection kind `%s`.', selection.kind)
        : invariant(false)
      : void 0;
  }
...
function printHandles(field) {
  ...
  var printed = field.handles.map(function(handle) {
    ...
    var filters =
      handle.filters == null
        ? ''
        : ', filters: '.concat(JSON.stringify(Array.from(handle.filters).sort()));
...
function printValue(value, type) {
  ...
  if (value.kind === 'Variable') {
    ...
    !(type instanceof GraphQLInputObjectType)
      ? process.env.NODE_ENV !== 'production'
        ? invariant(false, 'GraphQLIRPrinter: Need an InputObject type to print objects.')
        : invariant(false)
      : void 0;
    ...
    !(type instanceof GraphQLList)
      ? process.env.NODE_ENV !== 'production'
        ? invariant(false, 'GraphQLIRPrinter: Need a type in order to print arrays.')
        : invariant(false)
      : void 0;
