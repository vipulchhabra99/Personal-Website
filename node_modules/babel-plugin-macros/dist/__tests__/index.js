...
jest.mock('cosmiconfig', () => {
  ...
  Object.assign(mockSearchSync, {
    mockReset() {
      return mockSearchSync.mockImplementation((filename, configuredCosmiconfig) =>
  ...
  const _cosmiconfigMock = (...args) => ({
    searchSync(filename) {
      return mockSearchSync(filename, require.requireActual('cosmiconfig')(...args));
    },
  });
...
pluginTester({
  ...
  tests: [
      ...
      teardown() {
        try {
          ...
          expect(fakeMacro.innerFn).toHaveBeenCalledWith({
            ...
            source: expect.stringContaining('babel-plugin-macros-test-fake/macro'),
    ...
    {
      title: 'optionally keep imports in combination with babel-preset-env (#80)',
      code: `
        import macro from './fixtures/keep-imports.macro'
        const red = macro('noop')
      `,
    ...
    {
      title: 'appends the npm URL for errors thrown by node modules with a slash',
      error: true,
      code: `
        import errorThrower from 'babel-plugin-macros-test-error-thrower/macro'
        errorThrower('hi')
    ...
    {
      title: 'when there is an error reading the config, a helpful message is logged',
      error: true,
      fixture: path.join(__dirname, 'fixtures/config/code.js'),
      setup() {
    ...
    {
      title: 'when plugin options configuration cannot be merged with file configuration',
      error: true,
    ...
    {
      title: 'when a plugin that replaces paths is used, macros still work properly',
      fixture: path.join(__dirname, 'fixtures/path-replace-issue/variable-assignment.js'),
      babelOptions: {
        babelrc: true,
      },
    },
