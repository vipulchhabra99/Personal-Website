...
var getBlockName = function getBlockName(file) {
  ...
  return name !== 'index'
    ? name
    : _path.default.basename(_path.default.dirname(file.opts.filename));
};
...
var getDisplayName = function getDisplayName(t) {
  return function(path, state) {
    ...
    if (file) {
      ...
      return componentName
        ? `${(0, _prefixDigit.default)(blockName)}__${componentName}`
        : (0, _prefixDigit.default)(blockName);
    } else {
      return componentName;
    }
  };
...
var getFileHash = function getFileHash(state) {
  ...
  var filePath =
    moduleRoot && _path.default.relative(moduleRoot, filename).replace(separatorRegExp, '/');

  var moduleName =
    moduleRoot &&
    JSON.parse(_fs.default.readFileSync(_path.default.join(moduleRoot, 'package.json'))).name;
  var code = file.code;
...
var _default = function _default(t) {
  return function(path, state) {
    if (
      path.node.tag
        ? (0, _detectors.isStyled)(t)(path.node.tag, state)
        : /* styled()`` */
          ((0, _detectors.isStyled)(t)(path.node.callee, state) &&
            path.node.callee.property &&
            path.node.callee.property.name !== 'withConfig') || // styled(x)({})
          ((0, _detectors.isStyled)(t)(path.node.callee, state) &&
            !t.isMemberExpression(path.node.callee.callee)) || // styled(x).attrs()({})
          ((0, _detectors.isStyled)(t)(path.node.callee, state) &&
      ...
      var displayName =
        (0, _options.useDisplayName)(state) &&
        getDisplayName(t)(path, (0, _options.useFileName)(state) && state);
      addConfig(t)(
        path,
        displayName && displayName.replace(/[^_a-zA-Z0-9-]/g, ''),
        (0, _options.useSSR)(state) && getComponentId(state),
      );
    }
  };
};
