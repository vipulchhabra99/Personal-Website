...
var _default = function _default(t) {
  return function(path, state) {
    ...
    var id = path.scope.generateUidIdentifier(
      'Styled' +
        name.replace(/^([a-z])/, function(match, p1) {
    ...
    if (TAG_NAME_REGEXP.test(name)) {
      ...
      if (bindings[name] && !t.isImportDeclaration(bindings[name].path.parent)) {
        injector = function injector(nodeToInsert) {
          return (t.isVariableDeclaration(bindings[name].path.parent)
    ...
    if (t.isObjectExpression(css)) {
      ...
      css.properties = css.properties.reduce(function propertiesReducer(acc, property) {
        if (t.isObjectExpression(property.value)) {
          ...
          elem.node.attributes.push(
            t.jSXAttribute(t.jSXIdentifier(_name.name), t.jSXExpressionContainer(property.value)),
          );
      ...
      css.expressions = css.expressions.reduce(function(acc, ex) {
        if (
          Object.keys(bindings).some(function(key) {
          ...
          elem.node.attributes.push(
            t.jSXAttribute(t.jSXIdentifier(_name2.name), t.jSXExpressionContainer(ex)),
          );
