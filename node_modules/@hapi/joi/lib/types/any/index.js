...
module.exports = internals.Any = class {
  ...
  example(...examples) {
    ...
    for (let i = 0; i < examples.length; ++i) {
      ...
      const result = this._validate(
        value,
        localState,
        Settings.concat(internals.defaults, options.context ? { context: options.context } : null),
      );
      Hoek.assert(
        !result.errors,
  ...
  _validate(value, state, options, reference) {
    ...
    if (this._settings) {
      ...
      if (isDefaultOptions && this._settings[Symbols.settingsCache]) {
        ...
        options = Settings.concat(
          this._language ? Settings.concat({ language: this._language }, options) : options,
          this._settings,
        );
        if (isDefaultOptions) {
    ...
    if (
      this._flags.empty &&
      !this._flags.empty._validate(this._prepareEmptyValue(value), null, internals.defaults).errors
    ) {
      value = undefined;
    }
    ...
    if (presence === 'optional') {
      if (value === undefined) {
        const isDeepDefault =
          this._flags.hasOwnProperty('default') && this._flags.default === undefined;
  ...
  _finalizeValue(value, originalValue, errors, state, options) {
    ...
    if (
      ...
      (!this._flags.selfError || errors.some(e => state.path.length === e.path.length))
    ) {
