...
internals.String = class extends Any {
  ...
  _base(value, state, options) {
    if (typeof value === 'string' && options.convert) {
      ...
      if (this._flags.case) {
        value =
          this._flags.case === 'upper' ? value.toLocaleUpperCase() : value.toLocaleLowerCase();
      }
  ...
  ip(ipOptions = {}) {
    ...
    if (ipOptions.version) {
      ...
      for (let i = 0; i < ipOptions.version.length; ++i) {
        ...
        Hoek.assert(
          Ip.versions[version],
          'version at position ' + i + ' must be one of ' + Object.keys(Ip.versions).join(', '),
        );
  ...
  uri(uriOptions) {
    ...
    if (uriOptions) {
      ...
      const unknownOptions = Object.keys(uriOptions).filter(
        key =>
          !['scheme', 'allowRelative', 'relativeOnly', 'allowQuerySquareBrackets'].includes(key),
      );
  ...
  guid(guidOptions) {
    ...
    const guidRegex = new RegExp(
      `^([\\[{\\(]?)[0-9A-F]{8}([:-]?)[0-9A-F]{4}\\2?[${versionNumbers ||
        '0-9A-F'}][0-9A-F]{3}\\2?[${
  ...
  hex(hexOptions = {}) {
    ...
    Hoek.assert(
      typeof hexOptions.byteAligned === 'undefined' || typeof hexOptions.byteAligned === 'boolean',
