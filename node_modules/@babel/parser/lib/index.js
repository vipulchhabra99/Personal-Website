...
class ExpressionParser extends LValParser {
  ...
  parseMaybeAssign(noIn, refShorthandDefaultPos, afterLeftParse, refNeedsArrowPos) {
    ...
    if (this.state.type.isAssign) {
      ...
      if (
        patternErrorMsg &&
        ((left.extra && left.extra.parenthesized) || left.type === 'ParenthesizedExpression')
      ) {
        this.raise(
          maybePattern.start,
  ...
  parseExprOp(left, leftStartPos, leftStartLoc, minPrec, noIn) {
    ...
    if (prec != null && (!noIn || !this.match(types._in))) {
      if (prec > minPrec) {
        ...
        this.finishNode(
          node,
          op === types.logicalOR || op === types.logicalAND || op === types.nullishCoalescing
            ? 'LogicalExpression'
            : 'BinaryExpression',
        );
        return this.parseExprOp(node, leftStartPos, leftStartLoc, minPrec, noIn);
  ...
  parseMaybeUnary(refShorthandDefaultPos) {
    if (
      ...
      (this.scope.inAsync || (!this.scope.inFunction && this.options.allowAwaitOutsideFunction))
    ) {
  ...
  parseObjPropValue(
    ...
    const node =
      this.parseObjectMethod(prop, isGenerator, isAsync, isPattern, containsEsc) ||
...
class StatementParser extends ExpressionParser {
  ...
  pushClassPrivateMethod(classBody, method, isGenerator, isAsync) {
    ...
    classBody.body.push(
      this.parseMethod(method, isGenerator, isAsync, false, false, 'ClassPrivateMethod', true),
    );
  }
  ...
  isAsyncFunction() {
    ...
    return (
      ...
      (next + 8 === this.length || !isIdentifierChar(this.input.charCodeAt(next + 8)))
    );
  }
