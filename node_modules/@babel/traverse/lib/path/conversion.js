...
function arrowFunctionToExpression({ allowInsertArrow = true, specCompliant = false } = {}) {
...
function hoistFunctionEnvironment(fnPath, specCompliant = false, allowInsertArrow = true) {
  ...
  const { thisPaths, argumentsPaths, newTargetPaths, superProps, superCalls } = getScopeInformation(
    fnPath,
  );
    ...
    if (superProp.node.computed) {
      ...
      assignmentPath
        ...
        .replaceWith(
          t().binaryExpression(
            op,
            t().memberExpression(superProp.node.object, t().identifier(tmp.name), true),
            value,
          ),
        );
    } else {
      assignmentPath
        .get('left')
        .replaceWith(t().memberExpression(superProp.node.object, superProp.node.property));
      assignmentPath
        .get('right')
        .replaceWith(
          t().binaryExpression(
    ...
    thisEnvFn.traverse({
      ...
      CallExpression(child) {
        ...
        child.replaceWithMultiple([
          child.node,
          t().assignmentExpression('=', t().identifier(thisBinding), t().identifier('this')),
        ]);
      },
    });
  });
