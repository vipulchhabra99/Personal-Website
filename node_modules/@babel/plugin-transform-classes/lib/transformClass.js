...
function transformClass(path, file, builtinClasses, isLoose) {
    ...
    if (instanceProps || staticProps) {
      ...
      body.push(
        _core().types.expressionStatement(
          _core().types.callExpression(classState.file.addHelper('createClass'), args),
        ),
      );
    }
  ...
  function wrapSuperCall(bareSuper, superRef, thisRef, body) {
    ...
    if (classState.isLoose) {
      ...
      bareSuperNode = (0, _helperOptimiseCallExpression().default)(
        _core().types.callExpression(classState.file.addHelper('getPrototypeOf'), [
          _core().types.cloneNode(classState.classRef),
        ]),
        _core().types.thisExpression(),
        bareSuperNode.arguments,
      );
      call = _core().types.callExpression(classState.file.addHelper('possibleConstructorReturn'), [
        _core().types.thisExpression(),
        bareSuperNode,
      ]);
    }
  ...
  function verifyConstructor() {
    ...
    for (const thisPath of classState.superThises) {
      ...
      thisPath.replaceWith(
        _core().types.callExpression(classState.file.addHelper('assertThisInitialized'), [
          thisRef(),
        ]),
      );
    }
  ...
  function setupClosureParamsArgs() {
    ...
    if (classState.isDerived) {
      const arg = classState.extendsNative
        ? _core().types.callExpression(classState.file.addHelper('wrapNativeSuper'), [
  ...
  function classTransformer(path, file, builtinClasses, isLoose) {
    ...
    body = body.concat(
      classState.staticPropBody.map(fn => fn(_core().types.cloneNode(classState.classRef))),
    );
