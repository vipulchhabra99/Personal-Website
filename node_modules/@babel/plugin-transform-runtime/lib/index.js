...
var _default = (0, _helperPluginUtils().declare)((api, options, dirname) => {
  ...
  if (![false, 2, 3].includes(corejsVersion)) {
    throw new Error(
      `The \`core-js\` version must be false, 2 or 3, but got ${JSON.stringify(rawVersion)}.`,
    );
  }
  ...
  if (absoluteRuntime !== false) {
    modulePath = resolveAbsoluteRuntime(
      moduleName,
      _path().default.resolve(dirname, absoluteRuntime === true ? '.' : absoluteRuntime),
    );
  }
    ...
    pre(file) {
      if (useRuntimeHelpers) {
        file.set('helperGenerator', name => {
          ...
          const blockHoist =
            isInteropHelper && !(0, _helperModuleImports().isModule)(file.path) ? 4 : undefined;
          const helpersDir =
            esModules && file.path.node.sourceType === 'module' ? 'helpers/esm' : 'helpers';
    ...
    visitor: {
      ReferencedIdentifier(path) {
        ...
        if (name === 'regeneratorRuntime' && useRuntimeRegenerator) {
          path.replaceWith(
            this.addDefaultImport(`${modulePath}/regenerator`, 'regeneratorRuntime'),
          );
          return;
        }
