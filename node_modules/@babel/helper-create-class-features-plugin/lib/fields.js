...
const privateNameHandlerSpec = {
  ...
  set(member, value) {
    const { classRef, privateNamesMap, file } = this;
    const { name } = member.node.property.id;
    const { id, static: isStatic, method: isMethod, setId } = privateNamesMap.get(name);
...
function buildFieldsInitNodes(ref, superRef, props, privateNamesMap, state, loose) {
  ...
  for (const prop of props) {
    ...
    switch (true) {
      ...
      case isInstance && isPrivate && isField && !loose:
        instanceNodes.push(
          buildPrivateInstanceFieldInitSpec(_core().types.thisExpression(), prop, privateNamesMap),
        );
        break;
      ...
      case isInstance && isPrivate && isMethod && !loose:
        instanceNodes.unshift(
          buildPrivateInstanceMethodInitSpec(_core().types.thisExpression(), prop, privateNamesMap),
        );
        staticNodes.push(buildPrivateMethodDeclaration(prop, privateNamesMap, loose));
        break;
