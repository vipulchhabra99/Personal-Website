      ...
      if (keys.length === 0) {
        ...
        if (!allLiteral) {
          keyExpression = _core().types.callExpression(
            _core().types.memberExpression(keyExpression, _core().types.identifier('map')),
      ...
      if (!pattern.properties.length) {
        this.nodes.push(
          _core().types.expressionStatement(
            _core().types.callExpression(this.addHelper('objectDestructuringEmpty'), [objRef]),
          ),
        );
      }
      ...
      for (let i = 0; i < pattern.elements.length; i++) {
        ...
        if (_core().types.isRestElement(elem)) {
          ...
          elemRef = _core().types.callExpression(
            _core().types.memberExpression(elemRef, _core().types.identifier('slice')),
      ...
      ForXStatement(path) {
        ...
        if (_core().types.isPattern(left)) {
          ...
          node.body.body.unshift(
            _core().types.expressionStatement(_core().types.assignmentExpression('=', left, temp)),
          );
          return;
        }
