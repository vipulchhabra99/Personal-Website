...
var _getPlatformSpecificDefault = _interopRequireDefault(
  require('./get-platform-specific-default'),
);
...
function _default({ types: t }, { include, exclude, polyfillTargets, debug }) {
  const polyfills = (0, _filterItems.default)(
    _corejs2BuiltIns.default,
    include,
    exclude,
    polyfillTargets,
    (0, _getPlatformSpecificDefault.default)(polyfillTargets),
  );
    ...
    MemberExpression: {
      enter(path) {
        ...
        if ((0, _utils.has)(_builtInDefinitions.InstanceProperties, propertyName)) {
          ...
          if (instanceType) {
            InstancePropertyDependencies = InstancePropertyDependencies.filter(module =>
    ...
    VariableDeclarator(path) {
      ...
      for (const _ref of id.properties) {
        ...
        if (
          !node.computed &&
          t.isIdentifier(key) &&
          (0, _utils.has)(_builtInDefinitions.InstanceProperties, key.name)
        ) {
