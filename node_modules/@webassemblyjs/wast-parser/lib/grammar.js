...
function parse(tokensList, source) {
  ...
  function walk() {
    ...
    function parseBlock() {
      ...
      while (token.type === _tokenizer.tokens.openParen) {
        ...
        if (lookaheadAndCheck(_tokenizer.keywords.result) === true) {
          ...
          lookaheadAndCheck(_tokenizer.tokens.name) === true ||
          lookaheadAndCheck(_tokenizer.tokens.valtype) === true ||
      ...
      while (token.type !== _tokenizer.tokens.closeParen) {
        ...
        if (state.registredExportedElements.length > 0) {
          state.registredExportedElements.forEach(function(decl) {
            moduleFields.push(
              t.moduleExport(decl.name, t.moduleExportDescr(decl.exportType, decl.id)),
            );
          });
    ...
    function parseFuncInstrArguments(signature) {
      ...
      while (
        token.type === _tokenizer.tokens.name ||
        isKeyword(token, _tokenizer.keywords.offset)
      ) {
        var key = token.value;
        eatToken();
      ...
      while (
        token.type !== _tokenizer.tokens.closeParen && // $FlowIgnore
    ...
    function parseFuncInstr() {
      ...
      if (token.type === _tokenizer.tokens.name || token.type === _tokenizer.tokens.valtype) {
        ...
        if (typeof object === 'undefined') {
          ...
          return t.withLoc(
            t.objectInstruction(_name2, object, _args, _namedArgs),
            endLoc,
            startLoc,
          );
        }
    ...
    function parseFunc() {
      ...
      while (
        token.type === _tokenizer.tokens.openParen ||
        token.type === _tokenizer.tokens.name ||
        token.type === _tokenizer.tokens.valtype
      ) {
