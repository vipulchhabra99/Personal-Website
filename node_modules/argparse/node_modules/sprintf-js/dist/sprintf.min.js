...
!(function(a) {
  function b() {
    var a = arguments[0],
      c = b.cache;
    return (
      (c[a] && c.hasOwnProperty(a)) || (c[a] = b.parse(a)), b.format.call(null, c[a], arguments)
    );
  }
  function c(a) {
      ...
      if (((p = c(a[h])), 'string' === p)) q[q.length] = a[h];
        ...
        if (((j = a[h]), j[2]))
          for (g = f[n], i = 0; i < j[2].length; i++) {
            if (!g.hasOwnProperty(j[2][i]))
              throw new Error(b("[sprintf] property '%s' does not exist", j[2][i]));
            g = g[j[2][i]];
          }
        else g = j[1] ? f[j[1]] : f[n++];
        if (
          ('function' == c(g) && (g = g()),
          e.not_string.test(j[8]) && e.not_json.test(j[8]) && 'number' != c(g) && isNaN(g))
        )
          throw new TypeError(b('[sprintf] expecting number but found %s', c(g)));
        ...
        e.json.test(j[8])
          ? (q[q.length] = g)
          : (!e.number.test(j[8]) || (r && !j[3])
              ? (s = '')
              : ((s = r ? '+' : '-'), (g = g.toString().replace(e.sign, ''))),
            (l = j[4] ? ('0' === j[4] ? '0' : j[4].charAt(1)) : ' '),
            (m = j[6] - (s + g).length),
            (k = j[6] && m > 0 ? d(l, m) : ''),
            (q[q.length] = j[5] ? s + g + k : '0' === l ? s + k + g : k + s + g));
      }
    return q.join('');
  }),
    (b.cache = {}),
    (b.parse = function(a) {
      for (var b = a, c = [], d = [], f = 0; b; ) {
        if (null !== (c = e.text.exec(b))) d[d.length] = c[0];
          ...
          if (null === (c = e.placeholder.exec(b)))
  ...
  'undefined' != typeof exports
    ? ((exports.sprintf = b), (exports.vsprintf = f))
    : ((a.sprintf = b),
      (a.vsprintf = f),
