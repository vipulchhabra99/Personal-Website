...
test('node 6 buffer indexOf tests', function(t) {
  ...
  // test hex encoding
  t.equal(bindexOf(new Buffer(b.toString('hex'), 'hex'), '64', 0, 'hex'), 3);
  t.equal(bindexOf(new Buffer(b.toString('hex'), 'hex'), new Buffer('64', 'hex'), 0, 'hex'), 3);

  // test base64 encoding
  t.equal(bindexOf(new Buffer(b.toString('base64'), 'base64'), 'ZA==', 0, 'base64'), 3);
  t.equal(
    bindexOf(new Buffer(b.toString('base64'), 'base64'), new Buffer('ZA==', 'base64'), 0, 'base64'),
    3,
  );

  // test ascii encoding
  t.equal(bindexOf(new Buffer(b.toString('ascii'), 'ascii'), 'd', 0, 'ascii'), 3);
  t.equal(
    bindexOf(new Buffer(b.toString('ascii'), 'ascii'), new Buffer('d', 'ascii'), 0, 'ascii'),
    3,
  );
  ...
  */
  ...
  t.equal(bindexOf(new Buffer(b.toString('binary'), 'binary'), 'd', 0, 'binary'), 3);
  t.equal(
    bindexOf(new Buffer(b.toString('binary'), 'binary'), new Buffer('d', 'binary'), 0, 'binary'),
    3,
  );
  t.equal(bindexOf(new Buffer('aa\u00e8aa', 'binary'), '\u00e8', 0, 'binary'), 2);
  t.equal(bindexOf(new Buffer('\u00e8', 'binary'), '\u00e8', 0, 'binary'), 0);
  t.equal(bindexOf(new Buffer('\u00e8', 'binary'), new Buffer('\u00e8', 'binary'), 0, 'binary'), 0);
...

  {
    ...
    t.equal(4, twoByteString.indexOf(new Buffer('\u03a3', 'ucs2'), -6, 'ucs2'));
    t.equal(-1, twoByteString.indexOf('\u03a3', -2, 'ucs2'));
  }

  var mixedByteStringUcs2 = new Buffer('\u039a\u0391abc\u03a3\u03a3\u0395', 'ucs2');
  ...
  t.equal(6, mixedByteStringUcs2.indexOf(new Buffer('bc', 'ucs2'), 0, 'ucs2'));
  t.equal(10, mixedByteStringUcs2.indexOf(new Buffer('\u03a3', 'ucs2'), 0, 'ucs2'));
  t.equal(-1, mixedByteStringUcs2.indexOf(new Buffer('\u0396', 'ucs2'), 0, 'ucs2'));

  {
    ...
    t.equal(0, twoByteString.indexOf('\u039a\u0391', 0, 'ucs2'), 'Lambda Alpha');
    t.equal(2, twoByteString.indexOf('\u0391\u03a3', 0, 'ucs2'), 'Alpha Sigma');
    t.equal(4, twoByteString.indexOf('\u03a3\u03a3', 0, 'ucs2'), 'Sigma Sigma');
    t.equal(6, twoByteString.indexOf('\u03a3\u0395', 0, 'ucs2'), 'Sigma Epsilon');
  }
  ...
  t.equal(1534, longBufferString.indexOf('AJABACA', 511), 'Long AJABACA, Second J');

  pattern = 'JABACABADABACABA';
  t.equal(511, longBufferString.indexOf(pattern), 'Long JABACABA..., First J');
  t.equal(1535, longBufferString.indexOf(pattern, 512), 'Long JABACABA..., Second J');

  // Search for a non-ASCII string in a pure ASCII string.
  var asciiString = new Buffer('arglebargleglopglyfarglebargleglopglyfarglebargleglopglyf');
